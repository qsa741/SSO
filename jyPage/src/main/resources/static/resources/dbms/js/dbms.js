	...
	$('#dbmsTree').tree({
		...
		onClick: function(node) {
			if (node.id == 'SCHEMA' && node.text != saveClick) {
				$('#include').empty();
				$('#include').load("/dbmsTool/schemaDetails");
				saveClick = node.text;
			} else if (node.id == 'TABLE' && node.text != saveClick) {
				$('#include').empty();
				$('#include').load('/dbmsTool/tableDetails');
			}
		},
...
function test() {
	$.get({
		url: 'http://10.47.39.102:8080/dbmsTool/test'
	});
}
...
function loadObjectTable(node) {
	var root = $('#dbmsTree').tree('getRoot', node.target);
	addTab(node.text, 'http://10.47.39.102:8080/dbmsTool/loadTable?schema=' + root.text + '&objectType=' + node.id + '&objectName=' + node.text);
